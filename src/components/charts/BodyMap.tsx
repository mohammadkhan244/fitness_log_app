...
function computeFeedback(
  ...
  switch (muscle) {
    ...
    case 'Forearms': {
      if (sets === 0) {
        bad = `No forearm volume logged. Grip is a transmission point — force generated by lats and biceps has to pass through the wrist to produce any pulling output. Zero forearm stimulus leaves that transmission untrained. Risk: wrist and elbow tendinopathy.`;
      } else if (p < 2) {
        bad = `${sets} sets (${p}% of volume) — grip is a transmission point for all pulling strength. At this ratio, forearm endurance will cap how much lat and bicep output you can actually use before the grip gives out. Risk: wrist and elbow tendinopathy.`;
      } else {
        good = `${sets} sets (${p}%) — grip and wrist stability have active training stimulus.`;
      }
      if (sets > 0) {
        capable = `At ${sets} sets, forearms are a transmission point — force generated by lats, biceps, and upper back has to pass through the grip to produce pulling output. This ratio determines how much of your pulling strength actually transfers into usable output.`;
      }
      break;
    }
